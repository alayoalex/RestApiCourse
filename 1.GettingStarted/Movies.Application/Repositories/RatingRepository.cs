...
namespace Movies.Application.Repositories
{
    public class RatingRepository : IRatingRepository
    {
        ...
        public async Task<(float? Rating, int? UserRating)> GetRatingsAsync(Guid movieId, Guid? userId, CancellationToken token = default)
        {
        ...
        public async Task<bool> DeletingRatingAsync(Guid movieId, Guid userId, CancellationToken token)
        {
            using var connection = await _dbConnectionFactory.CreateConnectionAsync(token);
            var result = await connection.ExecuteAsync(new CommandDefinition("""
                delete from ratings
        ...
        public async Task<IEnumerable<MovieRating>> GetRatingsForUserAsync(Guid userId, CancellationToken token)
        {
            ...
            return await connection.QueryAsync<MovieRating>(new CommandDefinition("""
                ...
                from ratings r
                inner join movies2 m on r.movieid = m.id
                where userid = @userId
